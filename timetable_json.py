...
def main_teacher(input_index, name):
    ...
    try:
        ...
        for i, index in enumerate(teachers_index):
            if index not in ("", " "):
                url_teacher = (
                    "https://timetable.spbu.ru/WeekEducatorEvents/"
                    + index.strip()
                    + "/"
                )
                teacher_mas[name[i]] = {}
                teacher_mas, incorrect_time["teacher"] = info_schedule(
                    teacher_mas, name[i], url_teacher, incorrect_time["teacher"]
                )
        write_json_file("static/json/teacher.json", teacher_mas)
        write_incorrect_time(
            "static/json/incorrect_data.json", incorrect_time, "teacher"
        )
...
def main_student(input_index, name):
    ...
    try:
        ...
        for i, index in enumerate(student_index):
            if index not in ("", " "):
                url_student = (
                    "https://timetable.spbu.ru/MATH/StudentGroupEvents/Primary/"
                    + index
                    + "/"
                )
                student_mas[name[i]] = {}
                student_mas, incorrect_time["student"] = info_schedule(
                    student_mas, name[i], url_student, incorrect_time["student"]
                )
        write_incorrect_time(
            "static/json/incorrect_data.json", incorrect_time, "student"
        )
