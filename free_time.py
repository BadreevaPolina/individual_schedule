...
def estimate_time(
    end_time, begin_time, day, person, num, free_time_dict, flag, place_university
):
    ...
    if begin - end >= timedelta(hours=1, minutes=45):
        if flag == "True":
            places = ["", ""]
            add_json_free_time(
                day, begin_time, end_time, places, free_time_dict[person][num]
            )
        else:
            places = check_place(
                end_time, begin_time, day, person, num, place_university
            )
...
def check_place(
    end, begin, day, person, num, place_university
):  # end first lesson, begin second lesson
            ...
            for person1 in person1_dict:
                ...
                for person2 in person2_dict:
                    if person1["day_month"] == person2["day_month"]:
                        day_check = True
                        result_time = compare_place(
                            person1, person2, flag, place_university
                        )
                        if result_time != "":
                            time_b, time_e = break_time(result_time)
                            if time_b != time_e:
                                time_begin, time_end = result_time.split("-")
                                change_json_free_time(
                                    person1["day_month"],
                                    time_begin,
                                    time_end,
                                    place_university,
        ...
        for i in read["teacher"][k1]:
            ...
            if not day_check and with_day_check:
                ...
                if result_time != "":
                    ...
                    if time_begin != time_end:
                        add_json_answer(
                            i["day_month"], time_begin, time_end, answer_list, color, ""
                        )
...
def delete_repeat(timetable_unchanged_list):  # что делать если разные здания
    ...
    while i <= len(timetable_unchanged_list) - 1:
        answer1, answer2 = (
            timetable_unchanged_list[i].copy(),
            timetable_unchanged_list[i - 1].copy(),
        )
